...
Rect:            Generate rectanble or square mesh.
...
Falloff:         Terrain height falloff: Type 1, Type 2, X, Y, XY
Falloff Size:    Terrain falloff size.
                 This option is activated when Falloff type is X or Y.
Falloff X:       Terrain falloff size along X axis.
                 This option is activated when Falloff type is XY.
Falloff Y:       Terrain falloff size along Y axis.
                 This option is activated when Falloff type is XY.
...
###------------------------------------------------------------
# landscape_gen
def landscape_gen(x,y,z,meshsize_x, meshsize_y,
    ...
    nsize    = options[1]
    ntype      = int( options[2] )
    nbasis     = int( options[3] )
    vlbasis    = int( options[4] )
    ...
    offset     = options[10]
    gain       = options[11]
    marblebias     = int( options[12] )
    marblesharpnes = int( options[13] )
    marbleshape    = int( options[14] )
    invert       = options[15]
    height       = options[16]
    heightoffset = options[17]
    falloff      = int( options[18] )
    falloff_size_x = options[19]
    falloff_size_y = options[20]
    sealevel     = options[21]
    platlevel    = options[22]
    strata       = options[23]
    stratatype   = options[24]
    sphere       = options[25]
    ...
    elif ntype == 7: value = marble_noise( x*2.0/meshsize_x,y*2.0/meshsize_y,z*2.0*2/(meshsize_x+meshsize_y), origin, nsize, marbleshape, marblebias, marblesharpnes, distortion, depth, hardnoise, nbasis )
    ...
    if sphere == 0: # no edge falloff if spherical
        if falloff:
            ...
            fallofftypes = [0,
                            sqrt(ratio_x**4 + ratio_y**4),
                            sqrt(ratio_x**2 + ratio_y**2),
                            sqrt(ratio_y**falloff_size_y),
                            sqrt(ratio_x**falloff_size_x),
                            sqrt(ratio_x**falloff_size_x
                                 + ratio_y**falloff_size_y),
                           ]
...
# generate grid
def grid_gen( sub_d, size_me_x, size_me_y, options ):

    verts = []
    faces = []
    edgeloop_prev = []
    
    size_me_larger = size_me_x if size_me_x >= size_me_y else size_me_y
    delta = size_me_larger / float(sub_d - 1)
    sub_d_x = round(size_me_x / delta)
    sub_d_y = round(size_me_y / delta)

    start_x = -(size_me_x / 2.0)
    start_y = -(size_me_y / 2.0)

    for row_x in range(sub_d_x):
        edgeloop_cur = []
        x = start_x + row_x * delta
        for row_y in range(sub_d_y):
            y = start_y + row_y * delta
            z = landscape_gen(x, y, 0.0, size_me_x, size_me_y, options)
...
def sphere_gen( sub_d, size_me, options ):
    ...
    for row_x in range(sub_d):
        ...
        for row_y in range(sub_d):
            ...
            h = landscape_gen(u,v,w,size_me,size_me, options) / size_me
...
class landscape_add(bpy.types.Operator):
    ...
    bl_idname = "mesh.landscape_modified_add"
    bl_label = "Landscape Modified"
    ...
    RectMesh = BoolProperty(name="Rect",
                default=False,
                description="Not square mesh")
    ...
    MeshSizeX = FloatProperty(name="Mesh Size X",
                min=0.01,
                max=100.0,
                default=2.0,
                description="Mesh size X")

    MeshSizeY = FloatProperty(name="Mesh Size Y",
                min=0.01,
                max=100.0,
                default=2.0,
                description="Mesh size Y")
    ...
    FalloffSize = FloatProperty(name="Falloff Size",
                min=1.0,
                max=10.0,
                default=2.0,
                description="Falloff Size")

    FalloffSizeX = FloatProperty(name="Falloff X",
                min=1.0,
                max=10.0,
                default=4.0,
                description="Falloff Size X")

    FalloffSizeY = FloatProperty(name="Falloff Y",
                min=1.0,
                max=10.0,
                default=4.0,
                description="Falloff Size Y")
    ...
    def draw(self, context):
        ...
        if not self.SphereMesh:
            box.prop(self, 'RectMesh')
        box.prop(self, 'SmoothMesh')
        box.prop(self, 'Subdivision')
        if self.SphereMesh or not self.RectMesh:
            box.prop(self, 'MeshSize')
        else:
            box.prop(self, 'MeshSizeX')
            box.prop(self, 'MeshSizeY')
        ...
        if not self.SphereMesh:
            ...
            if self.Falloff == '3' or self.Falloff == '4':
                box.prop(self, 'FalloffSize')
            elif self.Falloff == '5':
                box.prop(self, 'FalloffSizeX')
                box.prop(self, 'FalloffSizeY')
    ...
    def execute(self, context):
        ...
        if self.AutoUpdate is True:
            ...
            options = [
                ...
                self.Offset,          #17
                self.Falloff,         #18
                self.FalloffSizeX,  #19
                self.FalloffSizeY,  #20
                self.Sealevel,        #21
                self.Plateaulevel,    #22
                self.Strata,          #23
                self.StrataType,      #24
                self.SphereMesh       #25
                ]
            if self.Falloff != '5':
                options[19] = options[20] = self.FalloffSize

            # Main function
            if self.SphereMesh:
                # sphere
                verts, faces = sphere_gen(self.Subdivision, self.MeshSize, options)
            elif not self.RectMesh:
                # square grid
                verts, faces = grid_gen(
                    self.Subdivision, self.MeshSize, self.MeshSize, options)
            else:
                # rectangle grid
                verts, faces = grid_gen(
                    self.Subdivision, self.MeshSizeX, self.MeshSizeY, options)
...
def menu_func_landscape(self, context):
    self.layout.operator(landscape_add.bl_idname, text="Landscape Modified", icon="PLUGIN")
